...
**Output**: The package generates a variety of plots and tables that summarize the transmission clusters, including phylogenetic trees and comparative heatmaps. It also provides statistical analysis of the clusters, such as permutation tests to determine the significance of the clusters.

### Requirements

The input data format **must** be a FASTA file with the sequences being the parts of the genome that differ from the reference genome. The package requires R version 4.4 or higher. The R packages required are listed in the `DESCRIPTION` file of the package, and will be installed automatically when the package is installed.
