...
compute_basic_cluster_counts <- function(seqs, seq2pt, ip_seqs, ip_pt_seqs, dates, pt_trace) {
    ...
    basic_counts["Number_of_study_start_indexes"] <- length(setdiff(
        unique(seq2pt[seqs[seqs %in% ip_seqs]]),
        unique(seq2pt[seqs[dates[seqs] > trace_dates[1] + 3]])
    ))
...
analyze_convert_patients <- function(seqs, seq2pt, ip_pt_seqs, dates, pt_trace) {
    ...
    if (num_converts > 0) {
        # for each convert patient, find the earliest isolate date
        convert_pos <- vapply(
            convert_pt_ids,
            function(pt_id) {
                patient_seqs <- seqs[seq2pt[seqs] == pt_id]
        ...
        # in pt_trace, 1.5 indicates a positive surveillance day
        # find the earliest positive surveillance day for each convert patient
        ...
        # if there is no positive surveillance day, set the conversion day to the last day in the trace matrix
        ...
        convert_data$num_later_converts <- sum(later_mask)
    }
...
analyze_conversions_after_index <- function(
    ...
    names(conversion_stats) <- c(
        ...
        "Number_of_converts_after_ip_seq",
        "Number_of_initial_converts_after_ip_seq",
    ...
    if (basic_counts["Number_of_start_indexes"] > 0 && convert_data$num_converts > 0) {
        ...
        # number of converts that appear on or after that index date
    ...
    if ((sum(seqs %in% ip_pt_seqs) > 0) && convert_data$num_converts > 0) {
        ...
        if (!is.na(earliest_index_pos)) {
            conversion_stats["Number_of_converts_after_ip_seq"] <- sum(
                convert_data$convert_pos >= earliest_index_pos
            )
            conversion_stats["Number_of_initial_converts_after_ip_seq"] <-
                sum(
                    convert_data$convert_pos >= earliest_index_pos &
                        convert_data$convert_pos != Inf
                )
        } else {
            conversion_stats["Number_of_converts_after_ip_seq"] <- 0
            conversion_stats["Number_of_initial_converts_after_ip_seq"] <- 0
        }
    } else {
        conversion_stats["Number_of_converts_after_ip_seq"] <- 0
        conversion_stats["Number_of_initial_converts_after_ip_seq"] <- 0
...
analyze_location_overlaps <- function(
    ...
    pt_overlap <- function(pt_donor, pt_recipient, first_pos_vec, trace) {
        pt_donor <- as.character(pt_donor)
        pt_recipient <- as.character(pt_recipient)
        ...
        if (donor_date <= recipient_date) {
            time_range <- as.character(seq(from = donor_date, to = recipient_date, by = 1))
            # check if there's a day where pt_donor and pt_recipient share a location
            any(
                floor(trace[pt_donor, time_range]) == floor(trace[pt_recipient, time_range]) &
                    (floor(trace[pt_recipient, time_range]) > 0)
            )
        } else {
            FALSE
        }
...
cluster_property_perm_test <- function(
    ...
    mean_stat_names <- paste("Mean", colnames(cluster_props))
    ...
    for (i in seq_len(nperm + 1)) {
        # mean statistics
        perm_props[i, mean_stat_names] <- apply(prop_array[,, i], 2, mean, na.rm = TRUE)
