...
[`plot_clusters_phylo()`](https://rdrr.io/pkg/hospitraceRVisualize/man/plot_clusters_phylo.html)
...
`[hospitraceRVisualize::plot_jaccard_similarity_heatmap()]` function.
This function generates a heatmap showing the overlap between the
clusters by indicating the proportion of isolates in each cluster that
are also present in the clusters generated by the other method. Before
we can use this function, we need to remove any singleton clusters,
which are clusters that contain only one isolate – these are not very
...
The `hospitraceR` package also provides functions for statistical
analysis of the clustering results. For example, we can use the
[`intra_cluster_genetic_var_analysis()`](https://theabhirath.github.io/hospitraceR/reference/intra_cluster_genetic_var_analysis.md)
...
![](hospitraceR_files/figure-html/plot_cluster_stats-1.png)

We can clearly see very specific mutational signatures emerge in the
clusters when compared to the population as a whole. For more
information on statistical analysis of the clustering results, see the
statistical analysis vignette, `vignette("statistical_analysis")`.
